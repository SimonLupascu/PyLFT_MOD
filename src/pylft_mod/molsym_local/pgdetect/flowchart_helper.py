...
def find_rotation_sets(mol, SEAs):
    ...
    """
    import numpy as np
    from pylft_mod.molsym_local.molecule import SEA as SEAClass

    split_SEAs = []
    for sea in SEAs:
        elements = [mol.atoms[i] for i in sea.subset]
        unique_elements = list(dict.fromkeys(elements))  # order-preserving unique
        if len(unique_elements) == 1:
            split_SEAs.append(sea)
        else:
            for el in unique_elements:
                indices = [i for i in sea.subset if mol.atoms[i] == el]
                new_sea = SEAClass("", indices, np.zeros(3))
                split_SEAs.append(new_sea)
    ...
    for sea in SEAs:
        ...
        if length < 2:
            ...
            axis = normalize(mol.coords[sea.subset[0], :])
            if axis is not None:
                for n in [6, 5, 4, 3, 2]:
                    re = RotationElement(axis, n)
...
def find_rotations(mol, rotation_set):
    ...
    # FIX: use union instead of intersection to catch axes from
    # single-atom SEAs that would otherwise be filtered out
    rsi_intersect = rotation_set_intersection(rotation_set)
    rsi_union = [re for sea_set in rotation_set for re in sea_set]
    # deduplicate union
    rsi_unique = []
    for re in rsi_union:
        if re not in rsi_unique:
            rsi_unique.append(re)
    # test all candidates, keep valid ones
    out = []
    for i in rsi_unique:
...
def is_there_ortho_c2(mol, SEAs, paxis):
    ...
    for sea in SEAs:
        ...
        if len(unique_elements) == 1:
            ...
            for el in unique_elements:
                indices = [i for i in sea.subset if mol.atoms[i] == el]
                split_SEAs.append(SEAClass("", indices, np.zeros(3)))
    SEAs = split_SEAs
...
def is_there_sigmav(mol, SEAs, paxis):
    ...
    for sea_i in SEAs:
        for sea_j in SEAs:
            if sea_i is sea_j:
                continue
            for i in sea_i.subset:
                for j in sea_j.subset:
                    vi = normalize(mol.coords[i,:])
                    vj = normalize(mol.coords[j,:])
                    if vi is None or vj is None:
                        continue
                    normal = normalize(np.cross(vi, vj))
                    if normal is None:
                        continue
                    if issame_axis(normal, paxis):
                        continue
                    sigma = reflection_matrix(normal)
                    molB  = mol.transform(sigma)
                    if isequivalent(mol, molB):
                        axes.append(normal)
